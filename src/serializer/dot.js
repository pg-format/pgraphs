...
const serializeId = id => idPattern.test(id) ? id :
  "\"" +  id.replaceAll(/[\t\b\f\n\r]/g," ").replaceAll("\"", "\\\"") + "\""

const serializeKeyValue = (key, value) =>
  value.map(value => serializeId(key) + "=" + serializeId(value)).join(" ")
...
export default ({nodes, edges}) => {
  ...
  const serializeEdge = ({from, to, properties}) =>
    "  " + serializeId(from) + dir + serializeId(to) + serializeProperties(properties) + ";"

...
const serializeKeyValue = (key, value) => string(key) + ":"
...
export default ({ nodes, edges }) => {
  ...
  const serializeNode = ({ id, labels, properties }) => "(" + ids.map(id)
    + (labels.length ? `{${labels.map(string).join(",")}}` : "")
    + serializeProperties(properties) + ")"

  const serializeEdge = ({ from, to, directed, labels, properties }) => "(" + ids.map(from) + ")-"
